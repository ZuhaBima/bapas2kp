    ...
    <div class="mainpage">
      ...
      <table class="table" id="table">
        ...
        <tbody>
          ...
          $result = pg_query(
            $conn,
            'SELECT * FROM litmas  WHERE id_jenis_litmas = 4 AND id_jenis_klien= 1'
          );
          $result2 = pg_query(
            $conn,
            'SELECT nama_pegawai FROM pegawai INNER JOIN litmas ON pegawai.nip = litmas.nip WHERE id_jenis_litmas = 4 AND id_jenis_klien= 1'
          );
          $result3 = pg_query(
            $conn,
            'SELECT nama_lapas FROM lapas INNER JOIN litmas ON lapas.id_lapas = litmas.id_lapas WHERE id_jenis_litmas = 4 AND id_jenis_klien= 1'
          );
          $result4 = pg_query(
            $conn,
            'SELECT jenis_kasus FROM kasus INNER JOIN litmas ON kasus.id_kasus = litmas.id_kasus WHERE id_jenis_litmas = 4 AND id_jenis_klien= 1'
          );
          $result5 = pg_query(
            $conn,
            'SELECT nama_status_litmas FROM status_litmas INNER JOIN litmas ON status_litmas.id_status = litmas.id_status WHERE id_jenis_litmas = 4 AND id_jenis_klien= 1'
          );
          ...
          ?>
            ...
            <tr align="center">
              ...
              <td>
                <a class="bi bi-pencil-square bg-icon-primary" href="../adminbps/bka-statusar.php?id_litmas=<?= $row['id_litmas'] ?>"></a>
    </div>
    </td>
    <td>
      <a onclick="return confirm('Yakin menghapus data ini ?')" class="bi bi-trash-fill bg-icon-danger" style="color: red;" href="../adminbps/bka-hapusar.php?id_litmas=<?= $row['id_litmas'] ?>"></a>
    </td>
    </tr>
  <?php
          }
